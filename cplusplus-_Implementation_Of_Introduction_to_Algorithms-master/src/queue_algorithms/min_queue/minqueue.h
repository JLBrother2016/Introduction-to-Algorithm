    ...
    namespace QueueAlgorithm
    {
        ...
        */
        ...
        public:
            ...
            *
            */
            MinQueue(CompareType compare,GetKeyType getKey):_size(0),_compare(compare),_getKey(getKey){}    //初始化列表        
            //!显式构造函数
            /*!
            ...
            */
            ...
            {
                if(!_size)
                    return std::shared_ptr<T>();                    //最小堆大小为0，返回为空
                return _data[0];                                    //否则返回最小堆的首元素
            }
            ...
            */
            std::shared_ptr<T> extract_min()
            {
                ...
                auto result=_data[0];                               //取出最小堆中的最小值
                _data[0]=_data[_size-1];                
                _size--;                                            //堆的长度减1
                heapify(0);                                         //维护最小堆的性质
                if(_size<=_data.size()/4)                           //维护堆的大小，如果小于最小堆的1/4，则队列的容量设置为队列长度的一倍
                    _data.resize(_size*2+2);                        //队列长度加2，为了防止极端情况
                return result;
            }
            //!insert:向队列中插入一个元素
            /*!
            ...
            */
            ...
            {
                if(! element)  return -1;
                if(_size==_data.size()) _data.resize(_size*2+2);    //队列容量太小就扩展为原来的两倍     
                int index=_size;
                _size++;
                _data[index]=element;
                TKeyType& k=_getKey(element);
                TKeyType old_k=k;                                   //保留插入元素的键值
                k=unlimit<TKeyType>();                              //将带插入的键值设置为无穷大，并将带插入元素插入到队尾
                decreate_key(index,old_k);                          //缩减键值
                return index;                                       //返回应该插入的位置
            }
            //!is_empty:返回队列是否为空
            /*!
            * \return 队列是否为空
            ...
            */
            int index_inqueue(std::shared_ptr<T> element)
            {
                ...
                {
                    if(element==_data[index])               //返回元素是否存在于队列中
                        return index;
                }
                return -1;
            }
            ...
            */
            void decreate_key(std::size_t element_index,TKeyType new_key)
            {
                ...
                if(new_key>_getKey(_data.at(element_index)))
                    throw std::invalid_argument("decreate_key error: new_key less _data.at(element_index)->key!");  //缩减值应该小于当前位置的值

                _getKey(_data.at(element_index))=new_key;
                bool valid;
                while(element_index!=0 )                                                    //不在队列首
                {
                    auto pIndex=_parentIndex(element_index,valid);                          //是否存在父节点
                    if(!valid) break;                                                       //不存在则直接返回
                    if(_compare(_data.at(pIndex),_data.at(element_index))) break;           //父节点和当前节点进行比较，父节点较小则终止迭代
                    std::swap(_data.at(pIndex),_data.at(element_index));                    //否则交换
                    element_index=pIndex;                                                   //当前节点设置为父节点，向上迭代
                }
            }
            //!setupHeap:建堆
            /*!
            ...
            */
            void setupHeap()
            {
                ...
                while(index>=0)
                {
                    heapify(index);                        //从后一半元素依次向前调用heapify操作
                    index--;
                }
            }
            //!heapify：维持堆性质
            ...
            * - 原地操作
            */
            void heapify(std::size_t elementIndex)                              //维持堆性质
            {
                if(elementIndex>=_size)                                         //越界直接返回
                    return;
                auto minIndex=elementIndex;
                bool left_valid=true;
                bool right_valid=true;
                auto leftIndex=_lchildIndex(elementIndex,left_valid);           //左孩子
                auto rightIndex=_rchildIndex(elementIndex,right_valid);         //右孩子

                if(left_valid)
                {
                    if( _compare(_data.at(leftIndex),_data.at(minIndex)))       //左节点比最小值还小
                        minIndex=leftIndex;                                     //最小值为左节点的值
                }
                if(right_valid)
                {
                    if( _compare(_data.at(rightIndex),_data.at(minIndex)))      //右节点的值比最小值小
                        minIndex=rightIndex;
                }
                if(minIndex!=elementIndex)                                      //最小值不等于该节点本身
                {
                    std::swap(_data.at(elementIndex),_data.at(minIndex));       //交换最小值和该节点的位置
                    heapify(minIndex);                                          //递归调用，维护最小堆性质
                }
            }
       protected:
            ...
            */
            std::size_t _parentIndex(std::size_t elementIndex,bool& valid)
            {
                ...
                else  return (elementIndex-1)>>1;                               //父节点的位置是该节点位置减1并除以2
            }
