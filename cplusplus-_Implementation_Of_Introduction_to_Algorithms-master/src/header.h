    ...
    */
    namespace GraphAlgorithm
    {
        ...
        */
        ...
        {
            return std::numeric_limits<T>::max()/2;     //正无穷取最大值的一半
        }
        //!is_unlimit：判断是否正无穷
        /*!
        * \param t: 待判断的数
        ...
        */
        ...
        {
            return t>=std::numeric_limits<T>::max()/3;  //大于最大值的3分之一都会认为是正无穷
        }
        ...
        */
        ...
        std::vector<typename VertexType::VIDType>       //返回类型，shared_ptr共享指针，可以多个指针指向
        get_path(const std::shared_ptr<VertexType> v_from, const std::shared_ptr<VertexType> v_to)
        {
            if(!v_from||!v_to)                          //要求`v_from`与`v_to`非空，否则抛出异常
                throw std::invalid_argument("get_path error: vertex must not be null!");
            std::vector<typename VertexType::VIDType> result;   //声明局部变量存储结果
            if(v_from->id==v_to->id)                            //起点等于终点，返回其中一个
            {
                result.push_back(v_from->id);
            }else if(!v_to->parent)
            {
                return std::vector<typename VertexType::VIDType>();  //v_to的父节点不存在则返回空
            }else                                                    //一般情况
            {                                                        //递归调用，起点不动，从子节点的父节点回溯  
                std::vector<typename VertexType::VIDType> temp=get_path<VertexType>(v_from,v_to->parent);
                for(const auto& val:temp)                            //每次递归的结果保存于result中
