...
//!BinaryTree：二叉树，算法导论10章10.4节
    ...
    */
    ...
    void  inorder_walk( std::shared_ptr<NodeType> root,ActionType action=[](typename NodeType::T){})
    {
        if (root)
        {
            if(root->lchild) inorder_walk(root->lchild,action); //递归调用
            action(root->key);                                  //action是一个函数对象
    ...
    *   左旋操作就是右孩子取代根节点，原来的根结点最为新的左孩子，r_node原来的右孩子成为node的右孩子，node的左孩子保持不变
    * <code><pre>
    *         |                                                |
    *        node                                            r_node
    *       /    \                                           /    \
    ...
    * 时间复杂度O(1)，空间复杂度O(1)
    */
    template<typename NodeType>                                         //左旋操作
    void left_rotate(std::shared_ptr<NodeType> node,std::shared_ptr<NodeType>& root)
    {
        ...
        if (node)                                                       //node非空
        {
            ...
            if(right)                                                   //r_node非空
            {
                ...
                {
                   if(node->is_left_child()) shared_p->lchild=right;    //让node的父节点重新指向新的子节点r_node
                   if(node->is_right_child()) shared_p->rchild=right;
                }else
                {
                    root=right;                                         //没有父节点，则证明node之前为根节点
                }
                node->rchild=l_right;                                   //r_node的左子节点剪切到node的右侧
                if(l_right) l_right->parent=node;                       //修改l_right的父节点为node

                node->parent=right;                                     //node剪切到r_node的左侧
                right->lchild=node;                                     //修改node的父节点为r_node，r_node的左孩子为node
               }
          }
    }
    ...
    * 时间复杂度O(1)，空间复杂度O(1)
    */
     template<typename NodeType>                                    //右旋和左旋类似
    ...
    * \return void
    *
    * 本函数执行二叉树的剪切操作。剪切操作要求目的节点是非空节点（若是空节点则抛出异常）。本操作将node_src剪切到node_dst中。下图中，两个节点之间有两个链接：
    * 一个是父-->子，另一个是子-->父
    * 剪切操作可能会仅仅剪断一个连接
    *
    *  <code><pre>
    ...
    * 时间复杂度O(1)，空间复杂度O(1)
    */
    template<typename NodeType>    //强引用
    void transplant(std::shared_ptr<NodeType> node_src,std::shared_ptr<NodeType>node_dst,std::shared_ptr<NodeType>& root)
     {
            if(!node_dst)
            {
            ...
            {
                node_src->parent=node_dst->parent;          //源节点的父节点为目标节点的父节点
            }
            auto shared_p=node_dst->parent.lock();          //将目标节点的父节点转换为强引用
            if(!shared_p)                                   //node_dst是根节点
            {
                root=node_src;                              //根节点直接为源节点
                return;
            }else                                           //node_dst不是根节点
            {
