...
TEST_F(BinaryTreeTest,test_inorder_walk)
{
    ...
    auto do_work=[&os](int val){ os<<val<<";";};   //do_work是一个函数对象，lambda表达式
...
TEST_F(BinaryTreeTest,test_left_rotate)
{
    ...
    left_rotate(_normal_tree.root,_normal_tree.root);   //左旋根节点
...
TEST_F(BinaryTreeTest,test_right_rotate)
{
    ...
    right_rotate(_normal_tree.root,_normal_tree.root);      //右旋根节点          
    EXPECT_NE(old_root.get(),_normal_tree.root.get());
