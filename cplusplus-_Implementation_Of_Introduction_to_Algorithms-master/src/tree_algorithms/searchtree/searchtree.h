...
namespace IntroductionToAlgorithm
{
    ...
    public:
            ...
            search(const T& value,std::shared_ptr<NodeType> node=std::shared_ptr<NodeType>())
            {
                if(!node) node=this->root;
                while(node&&node->key!=value)
                {
                    if(value<node->key)                     //小于在左节点
                    {
                        node=node->lchild;
                    }else                                   //大于在右节点
                    {
             ...
             */
            std::shared_ptr<NodeType> 
            successor(std::shared_ptr<NodeType> node)                   //后继，比该节点大一个单位
            {
                if(!node)
                {
             ...
             */
            std::shared_ptr<NodeType>
            predecesor(std::shared_ptr<NodeType> node)
            {
                if(!node)
                {
            ...
            {
                node->lchild=std::shared_ptr<NodeType>();           //为左节点申请空间
                node->rchild=std::shared_ptr<NodeType>();           //为右节点申请空间

                if(!this->root)                                     //这可搜索二叉树本身都没有根节点，让这个节点为根节点
                {
                    this->root=node;
                    return;
                }
                ...
                {
                    auto temp=this->root;                           //取出根节点    
                    auto temp_parent=std::shared_ptr<NodeType>();   //申请父节点的局部变量
                    bool left=true;                                 //初始化时，认为左节点存在
                    while(temp)                                     //从根节点遍历整个搜索二叉树
                    {
                    ...
                    node->parent=temp_parent;                       //插入节点的父节点为temp_parent
                    if(left) temp_parent->lchild=node;              //如果插入节点为左节点
                    else temp_parent->rchild=node;                  //插入节点为右节点
                }
             ...
             *
             */
            void remove(std::shared_ptr<NodeType> node)                                     //搜索二叉树的删除比较麻烦
            {
                if(!node)
                {
