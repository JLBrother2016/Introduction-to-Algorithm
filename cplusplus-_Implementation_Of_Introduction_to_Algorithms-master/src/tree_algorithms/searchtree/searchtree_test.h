    ...
    // ****  空树  ********
   {
    ...
    // *****  非空树 ************
    {
        ...
        _normal_tree.insert(node);
        {
            ...
            auto do_work=[&os](int val){ os<<val<<";";};            //lambda函数
            inorder_walk(_normal_tree.root,do_work);                //中序遍历
            EXPECT_EQ(os.str(),"0;1;2;3;4;5;6;7;8;1234;");
        }
        {
            std::ostringstream os;
            auto do_work=[&os](int val){ os<<val<<";";};
            preorder_walk(_normal_tree.root,do_work);               //前序遍历
...
TEST_F(SearchTreeTest,remove_test)
{
    ...
    // *****  非空树 ************
   {
        ...
        _normal_tree.remove(_normal_tree.root);                         //删除一个拥有两个子节点的节点，根节点5
        {
            std::ostringstream os;
            auto do_work=[&os](int val){ os<<val<<";";};
            inorder_walk(_normal_tree.root,do_work);
