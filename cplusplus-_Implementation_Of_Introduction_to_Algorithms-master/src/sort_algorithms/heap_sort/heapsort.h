...
namespace IntroductionToAlgorithm
{
    ...
    */
        ...
        public:
            ...
            * - 原地排序
            */
            void operator () (const Iterator from, std::size_t size,CompareType compare=CompareType())   //compare默认为小于
            {
                _from=from;
                _size=size;
                _setupHeap(compare);                                            //建堆
                while(_size>0)                                                  //堆的大小大于0，不断迭代
                {
                    std::swap(*_from,*(_from+_size-1));                         //交换堆的第一个元素和最后一个元素
                    _size--;                                                    //堆的规模缩减1
                    _heapify(0,compare);                                        //维持最大堆性质
                }
            }
        protected:
            //!_setupHeap:建堆
            ...
            */
            ...
            {
                if(_size<=1)                                                    //堆的大小小于1，直接返回
                    return;
                int index=(_size-1)/2;                                          //从后一半的元素开始一次向前调用heapify操作
                while(index>=0)
                {
            ...
            */
            ...
            {
                if(elementIndex>=_size)                                     //越界，直接返回
                    return;
                auto maxIndex=elementIndex;                                 //要维护的根节点
                bool left_valid=true;
                bool right_valid=true;
                auto leftIndex=_lchildIndex(elementIndex,left_valid);       //取出左孩子
                auto rightIndex=_rchildIndex(elementIndex,right_valid);     //取出右孩子

                if(left_valid)                                              //左孩子存在
                {
                    if( compare(*(_from+maxIndex),*(_from+leftIndex)))      //如果左孩子比根节点大，令根节点为左孩子
                        maxIndex=leftIndex;
                }
                if(right_valid)
                {
                ...
                if(maxIndex!=elementIndex)                                  //根节点的值已经改变
                {
                    std::swap(*(_from+elementIndex),*(_from+maxIndex));     //交换根节点和要交换节点的值
                    _heapify(maxIndex,compare);                             //向下递归保证每个节点均比左右孩子更大
                }
            }
            ...
            */
            std::size_t _parentIndex(std::size_t elementIndex,bool& valid)
            {
                ...
                else  return (elementIndex-1)>>1;                           //用右移1位表示除2，父节点是元素位置减1并除2
            }
            ...
            */
            std::size_t _lchildIndex(std::size_t elementIndex,bool& valid)
            {
                ...
                valid=true;
                return (elementIndex<<1)+1;                                 //左孩子的位置为当前位置/2再加1
            }
            ...
            */
            std::size_t _rchildIndex(std::size_t elementIndex,bool& valid)
            {
                ...
                valid=true;
                return (elementIndex<<1)+2;                                 //左孩子的位置为当前位置/2再加2
            }
        private:
